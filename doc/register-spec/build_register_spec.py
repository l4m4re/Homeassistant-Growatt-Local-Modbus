...
This is the Home Assistant compatibility and migration projection of the
project-independent Growatt register/protocol knowledge. The shared authority
is maintained in the
[`growatt-inverter-info`](https://github.com/l4m4re/growatt-inverter-info)
repository. Home Assistant is a consumer and a corroborating implementation,
not the source of shared register truth.

The local machine-readable artifact is [`growatt-register-spec.json`](growatt-register-spec.json), validated by [`growatt-register-spec.schema.json`](growatt-register-spec.schema.json). It is generated by the HA compatibility pipeline from the retained local source/evidence corpus. It must be synchronized deliberately against an accepted GII commit; it is not a second maintained semantic truth. See [`../REGISTER_KNOWLEDGE_WORKFLOW.md`](../REGISTER_KNOWLEDGE_WORKFLOW.md).
